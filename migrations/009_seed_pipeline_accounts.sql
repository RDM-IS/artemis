...
INSERT INTO public.organizations (name, type, industry, notes)
SELECT 'Dover Corporation', 'prospect', 'Industrial', 'Tier 1 target. Diversified industrial manufacturer.'
WHERE NOT EXISTS (SELECT 1 FROM public.organizations WHERE LOWER(name) = 'dover corporation');

INSERT INTO public.organizations (name, type, industry, notes)
SELECT 'TTI / Milwaukee Tool', 'prospect', 'Industrial', 'Tier 1 anchor. Power tools and outdoor products.'
WHERE NOT EXISTS (SELECT 1 FROM public.organizations WHERE LOWER(name) = 'tti / milwaukee tool');

INSERT INTO public.organizations (name, type, industry, notes)
SELECT 'Acme Corp', 'prospect', 'Industrial', 'Demo account. CPO Jane Smith.'
WHERE NOT EXISTS (SELECT 1 FROM public.organizations WHERE LOWER(name) = 'acme corp');

INSERT INTO public.organizations (name, type, industry, notes)
SELECT 'Stanley Black & Decker', 'prospect', 'Industrial', 'Tier 2. Use TTI outcomes as reference.'
WHERE NOT EXISTS (SELECT 1 FROM public.organizations WHERE LOWER(name) = 'stanley black & decker');

INSERT INTO public.organizations (name, type, industry, notes)
SELECT 'Fortive', 'prospect', 'Industrial', 'Tier 2. FBS culture, Forseti fit.'
WHERE NOT EXISTS (SELECT 1 FROM public.organizations WHERE LOWER(name) = 'fortive');

INSERT INTO public.organizations (name, type, industry, notes)
SELECT 'Illinois Tool Works', 'prospect', 'Industrial', 'Tier 3. Deprioritized — federated model.'
WHERE NOT EXISTS (SELECT 1 FROM public.organizations WHERE LOWER(name) = 'illinois tool works');

-- ═══════════════════════════════════════════════════════════════
-- Deals (linked to orgs above)
-- ═══════════════════════════════════════════════════════════════

INSERT INTO public.deals (org_id, name, gate, stage, value, notes)
SELECT o.id, 'Dover — Forseti Pilot', 0, 'Research',
...
INSERT INTO public.deals (org_id, name, gate, stage, value, notes)
SELECT o.id, 'TTI — Forseti Pilot', 1, 'Outreach',
...
INSERT INTO public.deals (org_id, name, gate, stage, value, notes)
SELECT o.id, 'Acme — Forseti Demo', 2, 'Meeting',
...
INSERT INTO public.deals (org_id, name, gate, stage, value, notes)
SELECT o.id, 'SBD — Forseti Pilot', 0, 'Research',
...
INSERT INTO public.deals (org_id, name, gate, stage, value, notes)
SELECT o.id, 'Fortive — Forseti Pilot', 0, 'Research',
...
INSERT INTO public.deals (org_id, name, gate, stage, value, notes)
SELECT o.id, 'ITW — Forseti Pilot', 0, 'Research',
...
INSERT INTO public.contacts (name, title, phone, notes, org_id)
SELECT
    'Greg Weddle',
