...
**WORK-DAY SCOPE BUILT 2026-09-21 (`feat/diet1-workday`) — migration 040 proposed, NOT yet applied.** What exists: `artemis/nutrition.py` (pre-fill, day/entry status, the 48h correction window, the deviation parser and the saved-food → USDA → Open Food Facts source order), `artemis/notion_meal_plan.py` (read-only Notion reader), the silent 00:15 `nutrition_prefill` cron job + the lock sweep, the morning check-in line, a bare-`fix` handler, and `tests/test_diet1.py` (46 tests).
...
- **BLOCKED ON TWO CREDENTIALS THAT DO NOT EXIST.** There is **no Notion token** (`rdmis/dev/notion-token`) and **no Notion code path anywhere else in the repo** — Artemis has never talked to Notion. There is also **no USDA key** (`rdmis/dev/usda-api-key`). Until the Notion token exists AND both databases are shared with the integration, the 00:15 job records `prefill_outcome = 'unavailable'` and pre-fills nothing — the specced degradation, not a failure. Open Food Facts needs no key, so tier 3 works today.
...
**YOGA-5 — softer voice, Sanskrit cues, mid-hold cues (2026-09-20). NEXT, not started.** Three parts, one PR after YOGA-4:
- *Voice*: prefer Ava (enhanced if present) → Allison → Samantha → platform default, matched on `voiceURI`/name; rate ≈ 0.85 and pitch ≈ 0.95 as named constants. iOS returns an empty `getVoices()` on the first call, so wait for `voiceschanged` and retry rather than silently falling back. A picker (English voices) + rate slider on the setup screen, localStorage, try/catch guarded.
- *Sanskrit*: each pose gains `sanskrit` (display) and `sanskrit_spoken` (phonetic — the engine mispronounces the proper spelling). The 7 s lead-in stays English with the side; the transition cue becomes "Move to Ashta Chandrasana", Sanskrit, no side, because the lead-in just gave it and the screen shows it. On screen: English name and side large, Sanskrit smaller beneath. Meditation and the Stretch Trainer block have no meaningful Sanskrit and fall back to English. **Blocked:** Ryan referred to a Sanskrit table that did not come through — ask for it rather than inventing the names.
- *Mid-hold cues*: an optional `cue_mid` per pose, spoken 12 s into the hold, voice only, one short line and then silence (12 s never collides with the 7 s lead-in on a 40 s hold). Toggle on the setup screen, default on, localStorage, try/catch guarded. Savasana and meditation get one cue at the start and then nothing. Ryan supplied the starting text — child's "Let your forehead rest, widen your knees", cobra "Draw your shoulders down and back", downward dog "Press the floor away, let your heels sink", standing forward bend "Soften your knees, let your head hang", high lunge "Front knee over the ankle, back leg long", crescent "Lift through your ribs, reach up", extended puppy "Melt your chest toward the floor", bridge "Press through your heels, open your chest", supine twist "Let the top shoulder drop toward the mat", wind release "Draw the knee closer, relax your neck", seated side bend "Lengthen first, then lean", seated twist "For a deeper stretch, look over your shoulder", seated mountain "Relax your shoulders, sit tall", easy pose "Settle in, soften your jaw", savasana "Let everything go heavy". His framing: what helps in week one is noise by week six.

**GD-STRENGTH-CUES — on-screen detail and rest-period voice prompts (2026-09-20). Specced, not started. Depends on YOGA-5's voice module.** gym-display, strength sessions only; the existing beeps stay and voice is added on top.
- *Target block* under the exercise name on the active-exercise screen: reps, RPE cap, and a third line derived from the cap by one table — 6 → "stop with ~4 reps left", 7 → "~3 reps left", 7.5 → "~2–3 reps left", 8 → "~2 reps left". Must fit both orientations without pushing the logger below the fold.
- *Voice prompt* in the rest period **before each new exercise**, once per exercise, never between sets of the same one and never before the first: "Next exercise is leg press. 12 reps at RPE 6. Your last sets averaged RPE 6 at 150 pounds for 12 reps." Spoken at the start of that rest period so it does not collide with the beeps, and **skipped entirely when the rest is shorter than the utterance** rather than talking over the next set.
- *"Your last sets averaged"* is the mean across **all** sets of that exercise in the most recent earlier session — not the top set. Weight to the nearest pound, reps to the nearest whole rep, RPE to the nearest 0.5; skipped sets excluded. 140 @ 5, 150 @ 6, 160 @ 7 → "averaged RPE 6 at 150 pounds for 12 reps." Bodyweight omits the weight; no RPE on any set omits that clause; no history at all → "First time logging this one."
- **The average is for the spoken sentence only** (Ryan, 2026-09-20). `/last_logged`, the on-screen "Last:" hint and the stepper prefill keep using the most recent set — the top set is the right thing to prefill against.
- Voice, rate, pitch and the mute toggle are shared with the flow player, so **YOGA-5 lands first**.
...
- **What the box actually uses.** Ten secrets read: the RDS credential (`rds!db-bfe5d90f-…`), `rdmis/dev/anthropic-api-key`, `mattermost`, `gmail-oauth`, `gmail-token`, `calendar-token`, `booking-links`, `crm-api-key`, `openweather-api-key`, and `acos/vault-repo`.
