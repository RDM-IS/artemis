...
**NUTRITION-2 — BUILT, NOT DEPLOYED (2026-09-26; `feat/nutrition-2`).** HEALTH-PRIORITY #3; the spec is in HEALTH-PRIORITY (PR #206). Systems only — menus are #4 and Ryan's.
- **Meal source by day kind** (`nutrition.day_kind`, `prefill_day`): a `meal planning` row **dated** that day wins on any kind (`notion_meal_plan.fetch_dated_day`); else work → `default day — work day`, travel → `default day — travel day` (**Ryan creates this row**); off (incl. leave) → no default, `no_plan`, empty. A day that already has logged entries is never replaced. **No migration:** the unpicked off day reuses `no_plan` rather than a new outcome (ENUM-EXPAND avoided); `not_work_day` survives only on old rows.
- **Free-text logging** (`artemis/meal_log.py`, handler `meal_log` after `nutrition_fix`, before the legacy `nutrition`): "for breakfast I had …", "I had … for lunch", "making … for dinner", "snacked on …", with `yesterday`. Items split on commas / `and` / `with` unless the phrase is a saved food; a container ("a fruit bowl that had …") becomes its contents. Every item resolves saved → USDA → Open Food Facts and is scaled by `portion_factor`; what can't be resolved or scaled is a question and the rest is stored. Ryan's own example parses into six items; against synthetic foods it stores four and asks about two.
- **Units — the latent bug fixed.** Every lookup now carries its macro `basis` (`portion` / `serving` / `100g`). A weight scales a per-100 g source; a count needs a portion; a count against per-100 g is a question. `+ banana` via USDA used to store 100 g as "one banana". Open Food Facts no longer mixes per-serving and per-100 g fields within one product. `fix` deviations use the same parser, so `+ 120 g banana` is 120 grams.
- **The reply:** each stored item (USDA / Open Food Facts matches name what they matched, so a wrong match is visible), the day's totals, **what's left against the open `nutrition.target`**, and up to two recipes for the next slot — recipes only, eaten-in-the-last-3-days excluded, fit to calories left, ranked by protein + fiber per calorie. `what's left` answers from `nutrition.*`.
- **Targets:** `set target` (propose → confirm) now also writes `nutrition.target` in the same transaction. `health.nutrition_target` is still written because the grocery-staples generator reads it; both retire together.
- **Ryan's inputs, none blocking:** (1) a USDA key at `rdmis/dev/usda-api-key` (free, api.data.gov) — without it whole foods fall to Open Food Facts, a product database that can match "strawberry" to a jam; (2) the `default day — travel day` row; (3) off-day recipes, or a dated `meal planning` row per off day; (4) a target.
- **Not done:** suggestions can't filter by course or travel-suitability — `nutrition.food` has no such column. **Proposed DDL (not in `migrations/`):** `ALTER TABLE nutrition.food ADD COLUMN course TEXT, ADD COLUMN tags TEXT[] NOT NULL DEFAULT '{}';` mirrored from the Notion `recipes.course` and a new multi-select `tags` (e.g. `travel`, `from-scratch`). Legacy `log <n> cal` still writes `health.nutrition_log` via the LLM path — retire it with the rest of `health.*`.
- **Deploy:** box only, no migration, no Lambda change. **Verify:** send Ryan's example; confirm `nutrition.entry` rows for today with `source` saved / usda / off and a reply naming what it couldn't resolve; send `what's left`.
