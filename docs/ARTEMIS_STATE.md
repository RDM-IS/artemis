...
| `wake` | 04:30 → 06:30 | 07:30 → 09:30 | health tier only — the wake post + pre-departure |
| `open` | 06:30 → 17:00 | 09:30 → 22:30 | everything |

Each boundary belongs to the local date it falls on: Friday night goes quiet at 17:00 and Saturday wakes at 07:30; Sunday night goes quiet at 22:30 and Monday wakes at 04:30. Weekend times are `WEEKEND_WAKE_TIME` / `WEEKEND_OPEN_TIME` / `WEEKEND_QUIET_HOURS_START`, read through `quiet_hours.wake_time_on` / `open_time_on` / `quiet_start_on`. Time-bound crons have `*_weekend` twins (`sat,sun`) that share the weekday job's once-per-day guard; the Sunday health review runs at the weekend open (09:30).
