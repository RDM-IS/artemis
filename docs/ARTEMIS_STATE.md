...
**CHECKIN-DEAD — subjective capture stopped on 2026-09-22 (found 2026-09-26; DIAGNOSTIC IN PROGRESS, nothing fixed).** `energy`, `soreness` and pain have not been recorded since 2026-09-22, and it went unnoticed because **`health.daily_state` still gets a row every day** — written by a different writer.
...
- **Still to establish (the diagnostic continues):** whether the check-in never asks, asks and the parser does not match the replies, parses and drops the write, or simply was not replied to. That requires reading the actual Mattermost posts and replies from 9/22 on, and the 0–5 parser's accepted forms and its behaviour on unrecognised input — a silent ignore would explain all three fields at once.
...
- **Still to establish:** whether a path exists to log a cardio session from the iPad at all, and what happens when a `cardio_z2` day is completed.
