...
**SESSION-LIB — start any session on demand. LOCATION-1 BLOCKER CLEARED 2026-09-26; now blocked on ad-hoc logging instead (see ADHOC-LOG below). Nothing built.** A launcher for sessions outside the schedule. It **replaces YOGA-LAUNCH rather than sitting beside it** — one launcher, not two — and carries YOGA-LAUNCH's logging rules forward.
...
- **Nothing exists toward it (surveyed 2026-09-26).** No launcher, no ad-hoc endpoint, no route serving a list of runnable sessions. `/api/health` has nine routes — `/today`, `/status`, `/log`, `/today/logged`, `/last_logged`, `/sessions`, `/plan`, `/ingest`, `/overview` — and `/sessions` serves logged history for the Status page, not a catalogue. Nothing carried over from YOGA-LAUNCH in code; only the rules in this entry.

**ADHOC-LOG — SESSION-LIB's logging rule has no path today (found 2026-09-26; diagnostic only, nothing built).** SESSION-LIB's one logging rule is that ad-hoc sets are REAL — they feed LAST and the effort trend. They cannot be recorded at all right now: **`POST /api/health/log` requires `plan_id` and returns `{"error": "plan_id_required"}` without it.** That guard was added deliberately when `_resolve_plan_id` was deleted (a log used to attach to whatever row happened to be open), and it is the right guard — but it means a session that is not on the plan has nowhere to go.
- **The database allows what the API refuses:** `health.session_log.plan_id` is nullable and **2 historical rows carry NULL**. So the constraint is entirely in the API layer.
- **Consequence for SESSION-LIB:** the launcher cannot be built end to end until this is settled. Either ad-hoc logs get a nullable `plan_id` path with an explicit marker, or they attach to a synthetic row, or SESSION-LIB only ever launches sessions that already have a plan row — which would gut the point of it.

**SESSION-LABELS — two `session_type` values have no builder and silently render as Rest / Mobility (found 2026-09-26; diagnostic only).** `health_office._build_inner` dispatches `recovery_flow`, `strength*`, `cardio_z2` and `rest`, then falls through to `return _rest(week_num)` for everything else. Two CHECK values land in that fallthrough:
- **`cardio_intervals` — 14 plan rows, weekly 2026-06-10 → 2026-09-09.** The fallthrough describes what a **rebuild today** would produce, not what those rows contain: they carry `display_name = "Bike Intervals"` and were written by the **retired ramp seeder**, which had a real intervals definition. That definition left with the ramp; `_build_inner` never gained one. **Each row does carry a log** — one `session_summary` apiece, but every one is `logged_via='inferred'` with `rpe_actual` 8.5, i.e. the nightly backstop marking an undebriefed session, not a real log. So: real rows, real content, orphaned builder, and no session Ryan ever logged.
- **`walk` — 0 plan rows.** In the DB CHECK but absent from `health_office.LEGAL_SESSION_TYPES`: WALK-RETIRE removed it from the program and left it in the constraint. A row typed `walk` would also build as Rest / Mobility, and `validate_rows` would reject it first.
- **The fallthrough is the defect, not the two values.** A new `session_type` that reaches the seeder without a dispatch branch produces a plausible mobility session rather than failing. See the ENUM-EXPAND cost note under SESSION-LIB.
...
- **ROWER-MOVE — the mechanics are settled, the DECISION IS NOT RECORDED HERE (2026-09-26).** Ryan referred to "the rower relocation decision" on 2026-09-26; no decision is written down in this repo, so it is **outstanding and must be captured before the inventory is edited** — do not infer it from this entry, which says "possible". What IS verifiable is the consequence, and it is not small:
  - **Rowing is the only modality that advances the progression.** `cardio_baseline.advances_baseline()` returns True for `row` and nothing else; `rowing_baseline()` filters `WHERE modality = 'row'`. Everything else runs the same session target as a substitute and deliberately does not move the number.
  - **So moving the rower to MSP home moves the progression with it.** Richfield would resolve to `bike` / indoor trainer — `is_substitute: True` — and **Richfield cardio would stop advancing the rowing baseline entirely**. MSP home, which resolves to the explicit no-cardio state today, would become the only place rowing progresses.
  - **And the baseline has no data to carry either way:** `modality` is populated on **0 of 175** `session_log` rows (CARDIO-UNLOGGED), so `rowing_baseline()` returns None — "no baseline yet" — wherever the rower physically is. The move changes which room can build one; it does not lose one, because there is none.
...
**PACKAGE-IDENTITY — the Lambda package covers runtime code only — DONE 2026-09-26 (option (a)).** `scripts/package_hash.sh` hashes the git tree ids of the four paths `api/deploy.sh` zips: `api/`, `knowledge/`, `migrations/`, **`tests/`**. Because `tests/` is in that list, a test-only commit moves the package identity and DRIFT-ALARM reports Lambda drift for a change that cannot affect runtime. **A drift alarm that fires on changes which cannot affect runtime trains you to ignore it** — the same reasoning that replaced the old commit-sha comparison with a package hash in the first place (CI-2).
- **Measured, not asserted (2026-09-26).** Of the last 25 commits on `main`, **13 touched a hashed path and 8 of those 13 were tests-only** — 62 % of package-hash movements were noise. Two of them were in this session, each forcing a Lambda redeploy that shipped byte-identical runtime code.
- **What blocked it, and how (Ryan chose option (a), 2026-09-26).** `api/app/main.py`'s `POST /admin/run-tests` shelled out to `/var/task/tests/test_phase1_schema.py`, which was the single named TEST-DB-GUARD exemption — so `tests/` was in the package **deliberately**, and dropping the path first would have 500ed the endpoint. The 296-line script became **`api/app/schema_check.py`**, a module returning data (`run_checks(conn=None) -> {ok, passed, failed, total, checks}`) with `format_report()` rendering the text the script used to print. The endpoint imports and calls it — no subprocess, no `sys.executable`, no `/var/task` path — and keeps its path, its auth and its `returncode`/`stdout`/`stderr` response shape, with the structured results added beside them. **It is runtime code because it is reachable over HTTP in production**; that is the whole argument for where it lives.
- **One implementation, tested rather than duplicated.** `conn` is injectable, so `tests/test_phase1_schema.py` is now a 12-test guarded unit test driving the same function with a fake connection: the contract, the accounting, all 16 results (15 numbered checks, 12 has an a/b pair), and that a broken schema is *reported* rather than raised. The live schema is asserted where that means something — against the real database, through the endpoint.
- **Proved, not assumed (2026-09-26).** A tests-only commit touching two test files leaves the hash at `989ca38a68e89e48`, **unchanged**. The control matters as much: a one-line change in `knowledge/` moves it to `1363697ac89c9f4e` and one in `api/app/` to `8093865d41ced470`, so the hash got narrower, not blind. `/admin/run-tests` was called against the live database after the redeploy and returned **16/16 passed**, `returncode` 0, with the stdout text unchanged — and left no rows behind (checks 11 and 15 still write and clean up; both probe queries return 0). **Package size barely moved: 24,456,490 → 24,220,834 bytes, 23.3 MB to 23.1 MB.** The earlier 2.4 MB estimate was wrong — it came from `du -sh tests`, which counts `__pycache__` the zip excludes, and test source compresses well. **The win is alarm fidelity, not bytes**, which is the reason to do it anyway: 8 of 13 hashed-path commits were reporting drift that could not affect runtime.
- **The two lists must stay identical.** `package_hash.sh`'s paths are the package's identity only while they are exactly what `deploy.sh` zips. If they diverge the hash stops being an identity and becomes a guess — `deploy.sh`'s dirty-tree check was narrowed in the same change for that reason. One known asymmetry, pre-existing and deliberate: the hash covers all of `api/` while the zip ships `api/app/`, because `api/requirements.txt` and `api/deploy.sh` determine the bundled dependencies and so do affect the package.
...
**CLASS-ATTRIBUTES — `equipment_class` says what an implement IS, not what it can DO (recorded 2026-09-26; NOT being fixed).** The class model and `can_hold()` will keep producing **confident false positives** for any movement that depends on an ADJUSTMENT rather than an implement, and a false positive here reads as fact on the iPad.

- **The example that found it.** *Incline DB press* resolves to class `dumbbell`. Richfield has dumbbells. The class check passes, `can_hold` is consulted and says nothing about it — and **Richfield's bench is flat**. "Incline" is a property of the bench, which no equipment class can express. Caught only because the A/B requirements list was written by hand, movement by movement; the automated check reported 2 of 7 Strength B movements native at Richfield when the truth is 1.
- **What can_hold() actually means: "is there an approved substitution table for this session here".** Nothing more. It is False for `rest` and `recovery_flow` at every non-office location — both seeded there constantly — because only the STRENGTH branch of `validate_rows` consults it. **Do not reuse it as "can this room run this session".** Recorded next to the function as well.
- **The others, found by walking every seeded movement (2026-09-26):**
  - **Bench presence.** *DB bench press*, *DB fly*, *1-arm DB row* — all class `dumbbell`, all needing a bench. A room with dumbbells and no bench passes.
  - **Bench angle.** *Incline DB press* — flat vs adjustable, as above.
  - **Bench back support, and seated-vs-standing.** *Seated DB shoulder press* is class `dumbbell`; "seated" is precisely what clears a 6.5 ft ceiling, and the class cannot say whether the room has a bench with an upright back.
  - **Cable attachment.** *Cable face pull (rope)* is class `cable`; a station with only a straight bar passes.
  - **Anchor height.** *Cable Pallof press* needs a chest-height anchor, *Single-arm cable row* an adjustable one. Both class `cable`; a floor-only anchor passes.
  - **Bar type — the sharpest one after the bench.** `barbell` at the office is an Olympic bar (`bar: 45`, to 285 lb); at Richfield it is a **curl bar** (`bar: 0`, to 70 lb). **Same class.** You cannot squat or bench-press a curl bar, and nothing in the model expresses that — the config captures the load RANGE but not what the bar can be used for.
  - **`machine` is a category, not a capability.** Eleven distinct seeded movements are class `machine` — leg press, lat pulldown, seated leg curl, leg extension, rear delt fly, pec fly, calf press, ab machine crunch, seated cable row, seated back extension. **A room with any one machine passes the class check for all of them.** This is the largest false-positive surface in the model.
  - **Specific apparatus.** *Captain's chair knee raise* is class `bodyweight`; *Stability-ball crunch* likewise. Bodyweight does not mean no equipment.
  - **Room constraints that exist as data but nothing reads.** `load_config.CONSTRAINTS` now holds `ceiling_ft` and `standing_overhead`, and neither `can_hold()` nor `class_for()` consults them, so an overhead movement at Richfield's 6.5 ft passes every check.
- **Two attributes ARE modelled, which shows the shape a fix would take:** the PowerBlock `step` (5 vs 2.5 lb) and the `plates` list both live on the load config and both travel on the row. An attribute becomes checkable when it is data on the location rather than an adjective in a name.
- **Why it is not being fixed now.** The honest fix is per-movement REQUIREMENTS (pattern + required attributes) rather than a single class per exercise — which is the shape of the A/B requirements list Ryan walked the rooms with on 2026-09-26. That is a model change touching the seeder, the pain ladder's substitution pool, both load-config consumers and gym-display's stepper. **Until then, a `can_hold` True or a passing class check is necessary but not sufficient, and any new non-office substitution table must be checked movement by movement by a human.**

- **RICHFIELD-CAPTURE — what the Monday inventory has to answer, and what is blocked on each (list closed 2026-09-26).** Four things consume it at once, and three of them have rows waiting:
  1. **`load_config.RICHFIELD`** — two open `TODO(richfield)` values: the **PowerBlock increment** (5 lb assumed; may be 2.5) and the **curl-bar plate set** (recorded as 10/10/5/5/2.5 to 70 lb, unconfirmed). Both are `step` / `plates` fields, so they become checkable the moment they are real — and both change what `lighter_load()` computes for the pain-2 rule.
  2. **`warmup.RICHFIELD`** — absent, so **9/29, 10/02 and 10/03 all carry `prep_unknown`** and render "Not configured for this location". One entry clears all three.
  3. **The A and B substitution tables** — `subs_for("richfield", "strength_a")` and `..."strength_b"` are both empty, which is why 9/29 and 10/03 are knowingly wrong. **Per CLASS-ATTRIBUTES, check A movement by movement rather than trusting `can_hold()`**: the incline-press false positive is exactly the shape the class model cannot see, and two more A movements have the same shape — *DB bench press* (class `dumbbell`, needs a bench) and *Cable face pull (rope)* (class `cable`, needs a rope attachment, not just a cable).
  4. **`standing_overhead` at 6.5 ft** — currently `False` with the SEATED-press question still open in the config comment. It interacts with *Seated DB shoulder press* in the C table, which is class `dumbbell` and needs a bench with an upright back — another CLASS-ATTRIBUTES case.
- **BROWN-DEER-INVENTORY — CONFIRMED 2026-09-26.** A treadmill, a yoga mat, bodyweight only, **7 ft ceiling**. No loadable implements, no bench, no anchors. Written to all three configs: `load_config.BROWN_DEER`, `cardio.INVENTORY["brown_deer"]` (treadmill, already correct — verified rather than changed), `warmup.BROWN_DEER` (**5 min treadmill, easy** / **5 min mat mobility**, so neither the office elliptical nor the Stretch Trainer can appear there).
  - **PRESENT-AND-EMPTY is not ABSENT, and the difference is load-bearing (Ryan, 2026-09-26).** `for_location("brown_deer")` returns `{"bodyweight": none, "cardio": none}` — *we know, and there is nothing to load*. `for_location("msp_home")` returns `None` — *we do not know*. Both make `has_numeric_load()` False, which is exactly why that helper must never be the gate on its own: `for_location()` is. Collapsing the two is how a room gets seeded with another gym's numbers.
  - **The loadable classes are deliberately unlisted**, not set to a zero range: a class absent from a location's config is not available there, and at Brown Deer that absence is measured. `bands` and `trx` are absent too — there is no wall mount and no strap, unlike Richfield, which has both.
  - **Ceilings are recorded data now, not comments (`load_config.CONSTRAINTS`).** Richfield **6.5 ft**, Brown Deer **7.0 ft**, office unmeasured. `standing_overhead` is three-state — Richfield `False` (decided), Brown Deer `None` (**undetermined**: 7 ft is marginal and depends on reach, and it is moot in that room because there is nothing to press), office `True`. **`None` is never read as permission.** Richfield's 6.5 ft had lived only in a code comment.
  - **Knowing the room does not make it hold a lift.** `can_hold("brown_deer", …)` is False for all three strength sessions and will stay False. Note `can_hold` also returns False for `rest` and `recovery_flow` at every non-office location — it only ever means "is there an approved substitution table", and only the strength branch of `validate_rows` consults it, so that is not a bug, but it is not a general capability answer either.
...
- **Unknown — ask Ryan:**
  ...
  - ~~Brown Deer's inventory~~ **CONFIRMED 2026-09-26 — and the guess was wrong.** Not "probably machines and cables": a **treadmill, a yoga mat, bodyweight only, 7 ft ceiling**. No loadable implements, no bench, no anchors for bands or a strap. Recorded in `knowledge/load_config.py` (`BROWN_DEER`), `knowledge/cardio.py` and `knowledge/warmup.py`. **No strength session can be seeded there** — not for want of a substitution table, but because there is nothing to load.
...
- **The load config has two consumers, not one.** `gym-display` (`LOAD_CONFIG`, `PLATES_PER_SIDE`, `OLYMPIC_BAR_LBS`, `SMITH_BAR_LBS`, `STEP_OVERRIDES`) **and the box** (`artemis/health_regions.py`: `DB_MIN/MAX/STEP`, `STACK_STEP`, `PLATES_PER_SIDE`, `OLYMPIC_BAR_LBS`, `SMITH_BAR_LBS`), which `lighter_load()` uses for the pain-2 rule — 80% of last, rounded down to a load the gym can make. A Richfield session whose config lives only on the client would have the box compute an office-shaped load for a PowerBlock. **`blocks.load_config` travels per row and both sides read it.**
- **What the client refactor actually costs.** `LOAD_CONFIG` and friends are read by exactly **two** app files — `weight-step.ts` (`weightStepFor`) and, through it, `InlineExerciseLogger` and `WorkoutScreen` (`isBodyweight`). The real weight is in the tests: **24 references in `tests/equipment-weight.test.ts`** and **9 in `tests/equipment-real-values.test.tsx`**, which assert against the office numbers as constants. The shape of the change: `weightStepFor(ex)` becomes `weightStepFor(ex, plan.blocks.load_config)`, with the current office values as the fallback when a row carries none, so every existing row keeps working unchanged. Roughly: one signature, two call sites, two test files rewritten to pass a config, and `reachableTotals` / `plateBreakdown` taking their plate list from the config they are given (they already take `plates` as a parameter). **It is a refactor of typed paths, not a config edit — but it is contained.**
- **Resolution order, as already agreed: location first, then pain.** The location resolver produces the session for that day's inventory; the pain/soreness ladder then applies **to the resolved session**. Concretely: `SUBSTITUTION_POOL` (`artemis/health_regions.py`, seven office machine names today) is **filtered to the location's inventory before pain picks a replacement**, a pain removal applies to the location substitute rather than to the exercise as written, and the test to write is the one already specced — *shoulder pain 4 at Richfield removes the SUBSTITUTED exercise, and its replacement also comes from Richfield's inventory*.
- **`bands` and `trx` join the class union with a no-numeric-load mode.** Both sides grow the union (`EquipmentClass` in `equipment.ts`, `_CLASS_RULES` in `health_regions.py`). They behave like `bodyweight` for the stepper — no load, reps only — but they are **not** bodyweight: they need their own names so the resolver can match a TRX row to a pulling pattern and a band pulldown to a vertical pull, and so `lighter_load()` returns None for them rather than inventing a number. gym-display's `WeightStep` already has `isBodyweight`; the cleanest shape is `loadMode: "numeric" | "none"` with `bands` / `trx` / `bodyweight` all `none`, keeping `isBodyweight` as a derived flag for the two call sites that read it.
- **Which session fits Richfield: C.** Judged against the inventory (PowerBlocks to 80, curl bar + 70 lb of plates, flat bench, TRX, bands on a wall mount, stability ball, rower, bike on a trainer, 6.5 ft ceiling):
  - **Strength C — three of six are native** (DB Romanian deadlift; Seated DB shoulder press, seated so the ceiling is not in play; Pec fly → DB fly on the flat bench), and the rest are one-step table substitutions. **Best fit.**
  - Strength A — five of six need substituting (only DB bench press is native).
  - Strength B — one native (DB goblet squat) and a **gap**: Incline DB press has no incline bench at Richfield and no table entry.
  - **Proposed Richfield C table:** DB Romanian deadlift → *unchanged* · Pec fly → **DB fly (flat bench)** · Single-arm cable row → **1-arm DB row (bench)** *(TRX row is the alternative; the bench version keeps a numeric load, which is better for progression)* · Seated DB shoulder press → *unchanged* · Calf press → **standing DB calf raise** · Ab machine crunch → **stability-ball crunch** *(no table entry today — this is the one new line)*.
- **The scheduling question this raises, which is Ryan's to answer.** SCHEDULE-2 puts all three lifts on office days and the wi Friday is now a rest morning. Turning it into Strength C means either a **fourth** lifting day in that week, or **moving** one of the office lifts to the farm. That is a program decision, not a resolver decision, and it is not part of this plan.
- **Still blocked on inventories Ryan owes: Brown Deer and MSP home.** Both are *morning* blockers only — the **evenings are unaffected**, since a Recovery Flow needs a mat and nothing else, and the 22 seeded evenings (home ×13, Brown Deer ×6, Richfield ×3) already work. What is blocked: a Brown Deer Saturday morning session — **and as of the 2026-09-26 inventory it is blocked permanently, not pending data: the room is a treadmill, a mat and bodyweight, so it holds cardio, flows and rest and no lift at all** — and an MSP home Sunday/Saturday morning session (nothing is recorded at all). Richfield is the only non-office location whose inventory is known, which is why it is the one this plan can act on.
- **Storage, unchanged from the original entry:** the registry lives in `acos.system_state` (no migration). `blocks.load_config` and per-exercise `equipment_class` are plan-row data, written at seed time.
...
**CHECKIN-DEAD — subjective capture stopped on 2026-09-22 (found 2026-09-26; DIAGNOSED — the fix is CHECKIN-GATE, #205).** `energy`, `soreness` and pain have not been recorded since 2026-09-22, and it went unnoticed because **`health.daily_state` still gets a row every day** — written by a different writer.

- **Measured, 2026-09-26.** 11 `daily_state` rows total. `energy`: **6 rows ever**, 2026-07-22 → **2026-09-22**. `soreness`: **3 rows ever**, 2026-09-17 → **2026-09-22**. `health.pain_pattern`: **0 rows, ever**. `health.reflection`: **0 rows**. `health.adjustments`: **0 rows**.
- **Two writers, and only one of them writes these fields.** `knowledge/watch_prefill.py`'s UPSERT writes `sleep_hrs`, `resting_hr` and `weight_lbs` only — it has no `energy` or `soreness` column in it at all. `health_checkin.store_checkin()` is the only writer of those two. So a day with sleep and resting HR but no energy is a day the WATCH prefill wrote and **the check-in did not**.
- **The `*_source` columns prove it row by row.** From 2026-09-21 on, every row reads `sleep_source='watch'`, `resting_hr_source='watch'`, `weight_source='watch'` with `energy` and `soreness` NULL — **except 2026-09-22, which is the only row carrying `manual` sources, `energy=4` and `soreness={'overall': 0}`.** 2026-09-22 is therefore the last day a check-in was stored, and the evidence is in the data rather than inferred from logs.
- **Diagnosed 2026-09-26 — mostly no replies, plus one real bug.** Mattermost `artemis-ryan` since 9/21 holds exactly two messages from Ryan: the 9/22 check-in (`Sleep 6, energy 4, soreness 0, 281.5`, parsed and stored) and a 9/23 weight-chart request. The wake post asked every day and the 05:15 nudge fired. So energy and soreness are missing because **no check-in was sent** — the prompt, parser and write all work. **The bug:** `classify()`'s gate had no pain or tightness words, so a pain report parsed cleanly and was dropped with no reply, and the prompt never asked for pain. Fixed by **CHECKIN-GATE (#205)**, which also found pain leaking across regions in comma-less replies. A bare positional energy (`281.5, 6.5, 4, sore 0`) is still dropped silently — open question in CHECKIN-GATE.
- **`pain_pattern` is downstream of this.** The 21:55 recompute (`artemis/health_patterns.py`) is the only writer, and it derives patterns from logged pain. With no pain input since the check-in stopped — and only 3 soreness rows ever — it has never had anything to recompute. **The pain ladder has never run on real input**, so PAIN-1 is live and untested against data.

**CARDIO-UNLOGGED — 36 cardio sessions planned, 1 logged, 0 with a modality (found 2026-09-26; diagnostic only).**
- **Measured:** `health.plan` holds **36 `cardio_z2` rows** (2026-06-06 → 2026-10-28) and **14 `cardio_intervals` rows** (2026-06-10 → 2026-09-09). `health.session_log` holds **one** `cardio_block` row, ever, dated **2026-06-05** — before the office program existed. `modality` and `device` are populated on **0 of 175** rows.
- **So CARDIO-LOC's column is live and nothing has ever written it.** Migration 043 is applied, the Lambda accepts both fields (#195), `knowledge/cardio.py` resolves them, and no cardio session has been logged since any of that shipped. The rowing baseline (`cardio_baseline.rowing_baseline()`) therefore returns None — "no baseline yet" — which is the correct answer to an empty table and not a fault.
- **The 14 `cardio_intervals` dates:** weekly Wednesdays 2026-06-10, 06-17, 06-24, 07-01, 07-08, 07-15, 07-22, 07-29, 08-05, 08-12, 08-19, 08-26, 09-02, 09-09. **Every one has a log, and every one is `logged_via='inferred'`** — the 21:50 backstop, `rpe_actual` 8.5, no duration. None was logged by hand. They predate the office program's 2026-09-16 start, so they are history; see SESSION-LABELS for the builder that no longer exists.
- **The path from the iPad EXISTS and is CARDIO-LOC-aware.** `LogPanel.saveCardio()` (gym-display) posts `log_type: "cardio_block"` with `duration_sec`, `distance_m`, `hr_avg`, `hr_peak`, `rpe_actual`, and reads `modality` / `device` straight off `plan.blocks.cardio`. It is reached from `WorkoutScreen` via an explicit "Open log panel" button — a `steady` row is not redirected to /status, so the workout screen does open for it.
- **What actually happened on the one completed cardio day.** 2026-09-22's `cardio_z2` row has exactly one log: a **`session_summary`**, `logged_via='manual'`, `rpe_actual` 5.0, and **`duration_sec`, `distance_m`, `hr_avg` and `modality` all NULL**. So the session was marked done through the summary and the cardio block was never filled in. That is the whole explanation for "36 planned, 1 logged": finishing a cardio day does not require the cardio panel, and the summary alone records that it happened and nothing about what happened.
- **A second gate, for older rows.** `saveCardio` takes the modality from `plan.blocks.cardio`, and **only 1 of 36 `cardio_z2` rows carries that key** — 2026-09-30, built after CARDIO-LOC. On any earlier row a correct cardio_block log would still store `modality: null`. Rows the seeder writes from now on carry it.
- **Diagnosed 2026-09-26 — a design gap, not a broken path.** The iPad path exists and is CARDIO-LOC-aware (`LogPanel.saveCardio()`, reached from the workout screen's "Open log panel" button). A cardio day can be finished through the session summary alone, which records that it happened and nothing about what happened — that is the whole of "36 planned, 1 logged". The fix is **CARDIO-REQUIRED** (HEALTH-PRIORITY #2).
...
- **DIETITIAN-DATA — the 14-day intake table cannot close, and the reason is structural (measured 2026-09-26).** The report's section 2 wants 14 days of intake. **3 days exist** — 2026-09-22, 23 and 24 — and **none is confirmed**: two are `locked_unconfirmed` (the 48h sweep fired) and one is `assumed`. Every entry in all three is the Notion default day, unmodified.
  - **Pre-fill is `msp_work`-only, and only 8 of the 14 cycle days are `msp_work`.** So a trailing **calendar** 14-day window can never be more than 8/14 populated — it does not close late, it does not close at all under the current scope.
  - **Counting only pre-filled days**, the 14th lands on **2026-10-21**: the leave week contributes nothing (9/25 → 10/03 all record `prefill_outcome=not_work_day`, and 10/04 is `travel`), pre-fill resumes **2026-10-05**, and the next eleven work days are 10/05, 06, 07, 08, 13, 14, 15, 16, 19, 20, 21.
  - **So the report needs one of three decisions before it can be built:** a weekend/non-work meal set (currently out of scope), a table that reports "no data" days honestly rather than needing 14 filled ones, or a window defined in work days rather than calendar days.
  - **Section 5's intensity split has no source either.** It depends on ZONE-1's bands, which are not built, and the spec says never estimate the split — so it must print "not yet tracked".
...
**TEST-DB-GUARD — no test may reach production — DONE 2026-09-19.** `knowledge/dbguard.py` refuses every real connection (knowledge.db pool, the Lambda API engine, the reseed / validate / seed / migration scripts) while a test runs: `ARTEMIS_TEST_NO_DB=1`, which every `test_*.py` in `tests/`, `artemis/` and `knowledge/` sets (enforced by `tests/test_db_guard.py`), or pytest. **Zero exemptions as of 2026-09-26.** There was one — `tests/test_phase1_schema.py`, the live-schema check behind the Lambda's `/admin/run-tests` — and it existed only because RUNTIME CODE was living in `tests/`: the file WAS the 15-check probe, and the endpoint ran it with `subprocess.run([sys.executable, "/var/task/tests/test_phase1_schema.py"])`, so it connected to the real database on purpose. The probe is `api/app/schema_check.py` now (PACKAGE-IDENTITY) and the test beside it drives `run_checks()` with a fake connection, so it sets the flag like every other test and needs **less** than the other live-schema tests, not more — `test_schema_drift` and `test_confirm_dispatch` set the flag and reach a database; this one reaches none. `EXEMPT` in `tests/test_db_guard.py` is an empty dict, and the guard test passes with it empty. Adding an entry back means naming which database and why. Audit (recording fake, empty-DB behaviour): `test_archive_gate` would write 8 `acos.audit_log` rows per run (never run on the box — 0 in prod); `test_scheduler_registry` wrote `checkin_open:2026-09-18` at 15:05 on 9/18 (same value the real wake had set); `test_health_seed`'s live tier would insert the legacy 137-row baseline plan into production (never ran — every date already has a row; now skipped under the guard); `tests/test_mattermost.py` posted "ACOS Phase 1 complete…" to the live channel on every run — 3 posts on 9/19 09:59 from the verification run; moved to `scripts/check_mattermost.py` with posting behind `--post`. Everything else writes nothing. The 64 prod rows from 9/19 were deleted (audit `test_row_cleanup`).
