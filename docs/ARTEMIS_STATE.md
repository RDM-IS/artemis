...
- **BLOCKED: one runtime path reaches into `tests/`.** `api/app/main.py`'s `POST /admin/run-tests` shells out to `/var/task/tests/test_phase1_schema.py`. That file is the **single named TEST-DB-GUARD exemption** — the intentional live-schema check — so `tests/` is in the package deliberately, not by accident. Dropping the path without moving that file would 500 the endpoint. Nothing else in `api/app/`, `knowledge/` or `migrations/` imports from `tests/` (grep clean).
- **Evidence for the decision:** `/admin/run-tests` has **not been invoked in 30 days** of `/aws/lambda/rdmis-crm-api` logs. `tests/` is 2.4 MB of the package against 1.1 MB for the three runtime paths combined.
- **Options, for Ryan:** (a) move the probe into `api/app/` and have the endpoint import and call it rather than shelling out — it is reachable over HTTP in production, so it IS runtime code and belongs with it; (b) delete `/admin/run-tests`, since `/admin/debug-schemas` already introspects the schema and nothing has called it; (c) drop `tests/` from `package_hash.sh` only, still shipping it — rejected as written, because the probe would then be runtime-reachable code whose changes do not move the hash, which is the exact hole the hash exists to close. **Precedent for (a)/(b):** `tests/test_mattermost.py` was moved to `scripts/check_mattermost.py` for the same reason — a thing that is not a test should not live in `tests/`.
