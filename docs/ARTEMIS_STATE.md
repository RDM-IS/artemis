...
- **Status 2026-09-21:** 130 ingest calls, latest 13:17Z. **The first real night arrived** (9/20→9/21) with all seven sleep metrics, and the pre-fill wrote 9/21's sleep 5.7 h, resting HR 64 and weight 282.0, all marked `watch`. **No workout has arrived yet** (`health.watch_workout` has 0 rows), so the workout match is unexercised.
- **The sleep assumption held, and the order mattered.** `sleep_total_sleep` = 5.73 h, matching core + deep + rem = 5.72 h. But **`sleep_asleep` and `sleep_in_bed` both report 0.00** for stage-based watch sleep — with `sleep_asleep` first in `SLEEP_METRICS`, the pre-fill would have written 0.0 h. Keep `sleep_total_sleep` first.
