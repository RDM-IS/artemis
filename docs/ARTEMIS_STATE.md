...
- **`cardio_intervals` — 14 plan rows, all 2026-06-10 → 2026-09-09, and NONE carries a logged set.** Every one of them built a session whose `display_name` was **"Rest / Mobility"** and whose `blocks.type` was `mobility`. The label existed, the definition never did. The dates are listed under CARDIO-UNLOGGED below.
...
- **The 14 `cardio_intervals` dates, none with a logged set:** all fall 2026-06-10 → 2026-09-09, and every one rendered as "Rest / Mobility" (SESSION-LABELS). They predate the office program's 2026-09-16 start, so they are history rather than a live problem.
