...
**PLAN-STATUS-DEBRIS — `health.plan.status` and `original_date` are half-dead (small; PROPOSED, not acted on).** Both came from migration 030 with the ramp engine. **Nothing in the codebase writes either.** 53 rows carry `status='missed'`, all dated 7/25–9/15 — the retired ramp wrote them; zero rows in the current program have it. `original_date` is populated on **0 rows**. Meanwhile the Lambda still **reads** `status`: `_plan_day_status` treats `plan.status = 'completed'` as done.
