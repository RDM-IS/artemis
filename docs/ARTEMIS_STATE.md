...
- **Workout match — design approved 2026-09-21; build when the first workout record arrives,** and verify it against the 9/21 evidence (05:21–05:54 CDT, ~217 kcal of active energy, ~104 average HR). A pure function in `knowledge/`, run at ingest and again from a box job (a workout can arrive before or after the sets are logged).
  ...
  - **Adjacent workouts (Ryan, 2026-09-22).** Once a session has its matched workout, one more may attach **before** it (ending ≤ 5 min before it starts) and one **after** it (starting ≤ 5 min after it ends). It must fit the plan's type, or be a warm-up/cool-down cardio kind (walk, elliptical, indoor cycling). Only the matched workout anchors, so it never chains. Two equally close candidates on one side attach neither. Why: 9/22's Z2 was 21 min on the bike, then an 8-min walk 11 s later, and the walk was reported rather than attached.
