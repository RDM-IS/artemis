...
Two tiers (mirrors test_guardrails.py):
  ...
  * LIVE integration tests (skipped unless a local Postgres is reachable) —
    ...
    unknown→stub, the draft/bless/edit/drop state machine (incl. "a draft never
...
class TestDeterministicNotOverridable(unittest.TestCase):
    ...
    def test_bless_without_review_falls_through(self):
        """`bless 1` with no pending review must NOT be handled here (returns
        False so it can reach the LLM) — the review-context gate."""
        main = artemis_main
        main._dossier_review_state.pop("c1", None)
        post = {"channel_id": "c1", "id": "p1", "message": "bless 1"}
        with patch.object(main, "_mm", MagicMock()):
            handled = main._handle_dossier_command(post, "bless 1")
...
class TestCaptureParsing(unittest.TestCase):
    def test_parse_met_with_names_topic(self):
        p = dossier._parse_met_with("met with jeremy & dennis about week-one logistics")
        self.assertEqual(p["names"], ["jeremy", "dennis"])
        self.assertEqual(p["topic"], "week-one logistics")
        self.assertIsNone(p["date"])

    def test_parse_met_with_explicit_date(self):
        p = dossier._parse_met_with("met with jennifer about x on 2026-06-01")
        self.assertEqual(p["date"], date(2026, 6, 1))
        self.assertEqual(p["topic"], "x")
    ...
    def test_not_met_with(self):
        self.assertIsNone(dossier._parse_met_with("hello there"))
...
class TestCaptureImmutability(unittest.TestCase):
    ...
    def test_raw_notes_byte_identical(self):
        ...
        with patch.object(dossier, "_find_dossiers",
            ...
            reply = dossier.capture_meeting(f"met with jennifer about x\n{notes}")
        self.assertEqual(captured["raw_notes"], notes)
...
class TestParseNumbersReuse(unittest.TestCase):
    """bless number parsing reuses main._parse_numbers (the E3 range/& parser)."""
...
class TestLiveCapture(LiveBase):
    def test_capture_end_to_end_and_immutability(self):
        ...
        with patch.object(dossier, "_llm_extract", return_value=extraction):
            reply = dossier.capture_meeting(f"met with jennifer about connector\n{notes}")
        self.assertIn("Captured meeting #1", reply)
        m = _one("SELECT * FROM acos.dossier_meeting WHERE meeting_id=1")
        self.assertEqual(m["raw_notes"], notes)                # byte-identical
...
class TestLiveStateMachine(LiveBase):
    ...
    def test_draft_never_closes_a_loop_until_blessed(self):
        d = self._mk("dennis", "Dennis Rowe")
        did = d["dossier_id"]
        # an existing blessed OPEN loop
    ...
    def test_bless_transitions(self):
        d = self._mk("dennis", "Dennis Rowe")
        did = d["dossier_id"]
        m = self._seed_meeting(did)
        ...
        dossier.bless_all(mapping)
        self.assertEqual(_one("SELECT count(*) c FROM acos.dossier_entry WHERE status='blessed'")["c"], 1)
    ...
    def test_draft_invisible_to_blessed_surface(self):
        d = self._mk("dennis", "Dennis Rowe")
        did = d["dossier_id"]
        m = self._seed_meeting(did)
        dossier._apply_draft(m, d, {"log_entry": "Draft-only note."})
        # show() blessed-only excludes the draft entry; --drafts includes it
    ...
    def test_edit_replaces_and_blesses(self):
        d = self._mk("dennis", "Dennis Rowe")
        did = d["dossier_id"]
        m = self._seed_meeting(did)
        ...
        self.assertEqual(row["status"], "blessed")
...
class TestLiveBrief(LiveBase):
    def test_brief_render(self):
        ...
        _w("INSERT INTO acos.dossier_entry (dossier_id, entry_date, entry_text, status, blessed_at) "
           "VALUES (%s,%s,'First intro call went well.','blessed', now())", (did, date(2026, 7, 1)))
    ...
    def test_draft_commitment_tagged_in_open_loops(self):
        ...
        commitments.add_commitment("blessed task", date(2026, 7, 20), status="active", dossier_id=d["dossier_id"])
        commitments.add_commitment("draft task", None, status="draft", dossier_id=d["dossier_id"])
        out = dossier.brief("jennifer")
        self.assertIn("blessed task", out)
